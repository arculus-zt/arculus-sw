...
def start_mission():
    ...
    return myjsonify({"message": "Mission started", "X_COORD": X_COORD, "Y_COORD": Y_COORD, "ENEMY_X": ENEMY_X, "ENEMY_Y": ENEMY_Y, "ENEMY_RADIUS": ENEMY_RADIUS})
...
def command_to_move():
    ...
    if is_within_enemy_radius(new_x, new_y):
        ...
        return myjsonify({"message": "Found air defense", "X_COORD": X_COORD, "Y_COORD": Y_COORD, "TURN_POINT_X": TURN_POINT_X, "TURN_POINT_Y": TURN_POINT_Y, "ENEMY_RADIUS": ENEMY_RADIUS, "ENEMY_X": ENEMY_X, "ENEMY_Y": ENEMY_Y})
    
    if is_within_dest_radius(new_x, new_y, move_distance):
        return myjsonify({"message": "Reached Destination", "X_COORD": X_COORD, "Y_COORD": Y_COORD})
...
def add_to_blocklist():
    ...
    if ip and ip not in blocklist:
        blocklist.append(ip)
        return myjsonify({"message": f"{ip} added to blocklist"}), 200
    else:
        return myjsonify({"message": "Invalid IP or already in blocklist"}), 400

@app.before_request
def check_ip_blocklist():
    requester_ip = request.remote_addr
    if requester_ip in blocklist:
        return myjsonify({"message": "Forbidden access"}), 403
...
if __name__ == '__main__':
    ...
    if CURRENT_MODE == NOAUTH:
        ...
        app.run(host='0.0.0.0', port=3050, ssl_context=context)